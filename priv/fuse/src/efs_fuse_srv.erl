...
-record (efs_fuse_srv, { inodes, names }).
...
init ([]) ->
    State = #efs_fuse_srv{ inodes = gb_trees:from_orddict ([ { 6, [] } ]),
...
getattr (_, X, _, State) ->
    case gb_trees:lookup (X, State#efs_fuse_srv.inodes) of
        { value, { Type, _ } } when (Type =:= port) or (Type =:= pid) or (Type =:= nodes) ->
            { #fuse_reply_attr{ attr = ?DIRATTR (X), attr_timeout_ms = 1000 }, State };
...
lookup (_, 2, BinName, _, State) ->
    ...
    case gb_trees:lookup ({ port, Name }, State#efs_fuse_srv.names) of
        { value, { Ino, _ } } ->
            { #fuse_reply_entry{ 
...
lookup (_, 3, BinName, _, State) ->
    Name = erlang:binary_to_list (BinName),
    case gb_trees:lookup ({ pid, Name }, State#efs_fuse_srv.names) of
        { value, { Ino, _ } } ->
            { #fuse_reply_entry{ 
                    fuse_entry_param = #fuse_entry_param{ ino = Ino,
                        generation = 1,  % (?)
...
lookup (_, 4, BinName, _, State) ->
    Name = erlang:binary_to_list (BinName),
    case gb_trees:lookup ({ system_item, Name }, State#efs_fuse_srv.names) of
        { value, { Ino, _ } } ->
            { #fuse_reply_entry{ 
                    fuse_entry_param = 
...
lookup (_, 5, BinName, _, State) ->
    Name = erlang:binary_to_list (BinName),
    case gb_trees:lookup ({ nodes, Name }, State#efs_fuse_srv.names) of
        { value, { Ino, _ } } ->
            { #fuse_reply_entry{ 
                    fuse_entry_param = #fuse_entry_param{ ino = Ino,
                        generation = 1,  % (?)
...
lookup (_, 6, BinName, _, State) ->
    Name = erlang:binary_to_list (BinName),
    case gb_trees:lookup ({ env_var, Name }, State#efs_fuse_srv.names) of
        { value, { Ino, _ } } ->
            { #fuse_reply_entry{ 
                    fuse_entry_param = #fuse_entry_param{ ino = Ino,
                        generation = 1,  % (?)
    ...
    take_while (
        ...
        safe_nthtail (Offset,
            [ #direntry{ name = ".", offset = 1, stat = ?DIRATTR (2) },
                #direntry{ name = "..", offset = 2, stat = ?DIRATTR (1) } ] ++
            [ { #direntry{ name = erlang:port_to_list (P), offset = N + 2 }, P } || 
                { N, P } <- index (erlang:ports ()) ])),

    { MappedList, NewState } = 
    lists:mapfoldl (
        fun ({ E = #direntry{ name = Name }, Port }, Acc) ->
                { Ino, NewAcc } = make_inode ({ port, Name }, Port, Acc),
    ...
    (fun (R, { Total, Max }) -> 
        ...
        safe_nthtail (Offset,
            [ #direntry{ name = ".", offset = 1, stat = ?DIRATTR (3) },
                #direntry{ name = "..", offset = 2, stat = ?DIRATTR (1) } ] ++
            [ { #direntry{ name = erlang:pid_to_list (P), offset = N + 2 }, { pid, P } } 
                || { N, P } <- index (Processes) ] ++
            [ { #direntry{ name = atom_to_list (R), offset = N + 2 + NumProcesses }, { registered, P } } 
                || { N, { P, R } } <- index ([ { P, R } || P <- Processes, 
                        { registered_name, R } <- [ erlang:process_info (P, registered_name) ] ]) ])),

    { MappedList, NewState } = 
    lists:mapfoldl (
        fun ({ E = #direntry{ name = Name }, { pid, Pid } }, Acc) ->
                { Ino, NewAcc } = make_inode ({ pid, Name }, Pid, Acc),
                { E#direntry{ stat = #stat{ st_ino = Ino } }, NewAcc };
            ({ E = #direntry{ name = Name }, { registered, Pid } }, Acc) ->
                { Ino, NewAcc } = make_inode ({ registered, Name }, Pid, Acc),
                { E#direntry{ stat = #stat{ st_ino = Ino } }, NewAcc };
            (E = #direntry{}, Acc) ->
                { E, Acc }
        end,
    ...
    DirEntryList = take_while (
        ...
        lists:nthtail (Offset,
            ...
            [ { #direntry{ name = atom_to_list (Item),
                        ...
                        st_size = StSize } }, Item } ||
                { N, { Item, Info } } <- index ([ { I, erlang:system_info (I) } || I <- Items, is_valid_system_info (I) ]), 
                StSize <- [ erlang:iolist_size (io_lib:format ("~p.~n", [ Info ])) ] ])),

    { MappedList, NewState } = 
    lists:mapfoldl (
        fun 
            ({ E = #direntry{ name = Name }, Item }, Acc) ->
                { Ino, NewAcc } = make_inode ({ system_item, Name }, Item, Acc),
                { E#direntry{ stat = #stat{ st_ino = Ino } }, NewAcc };
            (E = #direntry{}, Acc) ->
                { E, Acc }
        end,
...
readdir (_, 5, Size, Offset, _Fi, _, State) ->
    ...
    { MappedList, NewState } = lists:mapfoldl (
        fun 
            ({ E = #direntry{ name = Name }, Item }, Acc) ->
                { Ino, NewAcc } = make_inode ({ nodes, Name }, Item, Acc),
                { E#direntry{ stat = #stat{ st_ino = Ino } }, NewAcc };
            (E = #direntry{}, Acc) ->
                { E, Acc }
        end,
    ...
    DirEntryList = take_while (
        ...
        safe_nthtail (Offset,
            ...
            [ #direntry{ name = Var, 
                ...
                StSize <- [ erlang:iolist_size (io_lib:format ("~p.~n", [ Value ])) ] ])),

    { MappedList, NewState } = lists:mapfoldl (
        fun (E = #direntry{ name = Name }, Acc) ->
                { Ino, NewAcc } = make_inode ({ env_var, Name }, void, Acc),
                { E#direntry{ stat = #stat{ st_ino = Ino } }, NewAcc }
        end,
        State,
        DirEntryList),
...
readlink (_, X, _, State) ->
    case gb_trees:lookup (X, State#efs_fuse_srv.inodes) of
        { value, { registered, Name } } ->
            case gb_trees:lookup ({ registered, Name }, State#efs_fuse_srv.names) of
                { value, { _, Pid } } ->
...
env_var_attr (Name, State) ->
    { Ino, _ } = gb_trees:get ({ env_var, Name }, State#efs_fuse_srv.names),
...
lookup_nodes_item (Item, Node, State) ->
    case lists:member (Node, erlang:nodes (Item)) of
        true ->
            { Ino, NewState } = make_inode ({ nodes_item, { Item, Node } }, void, State),
            { #fuse_reply_entry{ 
                    fuse_entry_param = 
                    #fuse_entry_param{ ino = Ino,
                        generation = 1,  % (?)
                        attr_timeout_ms = 1000,
                        entry_timeout_ms = 1000,
                        attr = nodes_item_attr ({ Item, Node }, NewState) } },
                NewState };
        false ->
            { #fuse_reply_err{ err = enoent }, State }
    end.
...
make_inode (Name, Extra, #efs_fuse_srv{names=NT, inodes=IT} = State) ->
    case gb_trees:lookup (Name, NT) of
        { value, { Ino, Extra } } ->
            { Ino, State };
        none ->
            { Max, _ } = gb_trees:largest (IT),
            NewInodes = gb_trees:insert (Max + 1, Name, IT),
            NewNames = gb_trees:insert (Name, { Max + 1, Extra }, NT),
            { Max + 1, State#efs_fuse_srv{ inodes = NewInodes, names = NewNames } }
    end.
...
maybe_node_type (Name) ->
    case lists:member (Name, [ "visible", "hidden", "connected", "this", "known" ]) of
        true ->
...
port_item_attr ({ Name, Item }, State) ->
    { _, Port } = gb_trees:get ({ port, Name }, State#efs_fuse_srv.names),
    try
...
read_nodes_directory (Dot, Name, Size, Offset, State) ->
    { _, Item } = gb_trees:get ({ nodes, Name }, State#efs_fuse_srv.names),
    DirEntryList = take_while (
        fun (R, { Total, Max }) -> 
                Cur = case R of 
                    { E, _ } -> fuserlsrv:dirent_size (E);
    ...
    { MappedList, NewState } = lists:mapfoldl (
        fun 
            ({ E = #direntry{ stat = Stat }, Node }, Acc) ->
                { Ino, NewAcc } = make_inode ({ nodes_item, { Item, Node } }, void, Acc),
                { E#direntry{ stat = Stat#stat{ st_ino = Ino } }, NewAcc };
            (E = #direntry{}, Acc) ->
                { E, Acc }
        end,
        State,
    ...
    DirEntryList = take_while (
        ...
        safe_nthtail (Offset,
            ...
            [ { #direntry{ name = atom_to_list (Item),
                        ...
                        stat = #stat{ st_mode = ?S_IFREG bor 8#0444,
                            st_size = StSize } }, Item } ||
                { N, { Item, Info } } <- index (erlang:process_info (Pid)),
                StSize <- [ erlang:iolist_size (io_lib:format ("~p.~n", [ Info ])) ] ])),

    { MappedList, NewState } = lists:mapfoldl (
        fun 
            ({ E = #direntry{ stat = Stat }, Item }, Acc) ->
                { Ino, NewAcc } = make_inode ({ pid_item, { Name, Item } }, void, Acc),
                { E#direntry{ stat = Stat#stat{ st_ino = Ino } }, NewAcc };
            (E = #direntry{}, Acc) ->
                { E, Acc }
        end,
    ...
    DirEntryList = take_while (
        ...
        safe_nthtail (Offset,
            ...
            [ { #direntry{ name = atom_to_list (Item),
                        ...
                        stat = #stat{ st_mode = ?S_IFREG bor 8#0444,
                            st_size = StSize } }, Item } ||
                { N, { Item, Info } } <- index (erlang:port_info (Port)),
                StSize <- [ erlang:iolist_size (io_lib:format ("~p.~n", [ Info ])) ] ])),

    { MappedList, NewState } = lists:mapfoldl (
        fun 
            ({ E = #direntry{ stat = Stat }, Item }, Acc) ->
                { Ino, NewAcc } = make_inode ({ port_item, { Name, Item } }, void, Acc),
                { E#direntry{ stat = Stat#stat{ st_ino = Ino } }, NewAcc };
            (E = #direntry{}, Acc) ->
                { E, Acc }
        end,
...
read_system_item (Name, Size, Offset, State) ->
    { _, Item } = gb_trees:get ({ system_item, Name }, State#efs_fuse_srv.names),
    try
...
%-                           fuse mothods
%-=====================================================================-
% open(Ctx, Inode, Fi, Cont, State) -> ok.
% getattr(Ctx, Inode, Cont, State) -> ok.
% read(Ctx, Inode, Size, Offset, Fi, Cont, State) -> ok.
% lookup(Ctx, Parent, Name, Cont, State) -> ok.
% readdir(Ctx, Inode, Size, Offset, Fi, Cont, State) -> ok.
% readlink(Ctx, Inode, Cont, State) -> ok.

%-spec create(#fuse_ctx{}, any(), any(), any(), 
%    #fuse_file_info{}, any(), any()) -> ok.

%create(Ctx, Parent, Name, Mode, Fi, Cont, State) -> ok.
%release(Ctx, Inode, Fi, Cont, State) -> ok.
%write(Ctx, Inode, Data, Offset, Fi, Cont, State) -> ok.
%fsync(Ctx, Inode, DataSync, Fi, Cont, State) -> ok.
%flush(Ctx, Inode, Fi, Cont, State) -> ok.
%rename(Ctx, Parent, Name, NewParent, NewName, Cont, State) -> ok.
%access(Ctx, Inode, Mask, Cont, State) -> ok.
%setattr(Ctx, Inode, Attr, ToSet, Fi, Cont, State) -> ok.
%link(Ctx, Ino, NewParent, NewName, Cont, State) -> ok.
%unlink(Ctx, Inode, Name, Cont, State) -> ok.

%mkdir(Ctx, Parent, Name, Mode, Cont, State) -> ok.
%rmdir(Ctx, Inode, Name, Cont, State) -> ok.
%opendir(Ctx, Inode, Fi, Cont, State) -> ok.
%releasedir(Ctx, Inode, Fi, Cont, State) -> ok.
%fsyncdir(Ctx, Inode, DataSync, Fi, Cont, State) -> ok.

%statfs(Ctx, Inode, Cont, State) -> ok.

%setxattr(Ctx, Inode, Name, Value, Flags, Cont, State) -> ok.
%getxattr(Ctx, Inode, Name, Size, Cont, State) -> ok.
%listxattr(Ctx, Ino, Size, Cont, State) -> ok.
%removexattr(Ctx, Inode, Name, Cont, State) -> ok.

%symlink(Ctx, Link, Inode, Name, Cont, State) -> ok.
%mknod(Ctx, Parent, Name, Mode, {Major, Minor}, Cont, State) -> ok.

%getlk(Ctx, Inode, Fi, Lock, Cont, State) -> ok.
%setlk(Ctx, Inode, Fi, Lock, Sleep, Cont, State) -> ok.
%forget(Ctx, Inode, Nlookup, Cont, State) -> ok.
%bmap() -> ok.
...
test_proc (LinkedIn) ->
    ...
    { setup,
        fun () ->
                ...
                ok = application:load (efs_fuse),
                Dir = "mount.tmp." ++
                integer_to_list (MegaSec) ++ "." ++
                integer_to_list (Sec) ++ "." ++
                integer_to_list (MicroSec),

                application:set_env (efs_fuse, linked_in, LinkedIn),
                application:set_env (efs_fuse, mount_point, Dir),
                application:set_env (efs_fuse, mount_opts, ""),
                ok = application:start (efs_fuse),
                Dir
        end,
        fun (Dir) -> 
                application:stop (efs_fuse),
                application:unload (efs_fuse),
